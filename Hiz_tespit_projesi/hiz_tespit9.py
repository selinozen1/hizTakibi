...
if __name__ == "__main__":
    ...
    last_speeds = {}  # Araç ID'sine göre son ölçülen hızları saklamak için


    # video kayıt için fourcc ve VideoWriter tanımlama
    cv2_fourcc = cv2.VideoWriter_fourcc(*'mp4v')
    size = list([648, 1152, 3])
    del size[2]
    size.reverse()
    video = cv2.VideoWriter("cikti_videosu.mp4", cv2_fourcc, 24, tuple(size))
    ...
    for frame in frame_generator:
        ...
        for tracker_id in detections.tracker_id:
            if len(coordinates[tracker_id]) < video_info.fps / 2:
                ...
                last_speeds[tracker_id] = int(speed)
        ...
        if len(detections.tracker_id) == len(detections.xyxy):
            ...
            annotated_frame = trace_annatator.annotate(scene=annotated_frame, detections=detections)
            annotated_frame = bounding_box_annotator.annotate(
                scene=annotated_frame, detections=detections)

            annotated_frame = label_annatator.annotate(
    ...
    with open("arac_hizi.txt", "w") as f:
        for tracker_id, speed in last_speeds.items():
            f.write(f"ID: {tracker_id}, Hiz: {speed} km/h\n")
